...
qboolean BotMP_CheckEmergencyGoals(bot_state_t *bs)
{
	...
	if (level.captureFlagMode)
	{
			...
			if (!BotFlagAtBase(bs->sess.sessionTeam, &flag))
			{
				...
				if (flag &&
				    (VectorDistanceSquared(flag->r.currentOrigin, bs->eye) < (1024 * 1024) ||
				     trap_InPVS(flag->r.currentOrigin, bs->eye) || BotNumTeamMatesWithTarget(bs, flag->s.number, NULL, 0) < 1))
				{
		...
		else					// if we are close to the flag, get it!
		{
			...
			if (((bs->leader == -1) || !trap_InPVS(bs->origin, g_entities[bs->leader].r.currentOrigin) ||
			     (VectorDistanceSquared(bs->origin, g_entities[bs->leader].r.currentOrigin) >
			      (MAX_BOTLEADER_DIST * MAX_BOTLEADER_DIST))) && (!BotCarryingFlag(bs->client)) &&
		...
		if ((numList = BotGetTargetDynamite(list, 10, NULL)))
		{
			for (i = 0; i < numList; i++)
			{
				...
				if (bs->sess.playerType != PC_ENGINEER)
				{
					...
					{
						if (level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] !=
						    BotGetTargetExplosives(bs->sess.sessionTeam, NULL, 0, qfalse) ||
				...
				if ((areanum = BotPointAreaNum(trav->s.number, center)))
				{
					...
					if (trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, areanum, bs->tfl))
					{
						...
						else if ((BotNumTeamMatesWithTargetAndCloser(bs, target.entitynum, target.areanum, NULL, 0, -1) <
		...
		switch (res)
		{
			...
			case BPG_NEWGOAL:
				...
				findGoals[i].numPlayers = numCloser =
				                              BotNumTeamMatesWithTargetAndCloser(bs, goal.entitynum, goal.areanum, NULL, 0,
...
qboolean BotMP_FindGoal(bot_state_t *bs)
{
...

	{
		...
		while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_TRIGGER_MULTIPLE)))
		{
			...
			if ((numList =
			         BotNumTeamMatesWithTargetAndCloser(bs, trav->s.number, target.areanum, list, MAX_CLIENTS,
			                 -1)) > (int)floor(0.3 * numTeammates))
			{
		...
		while ((trav = BotFindNextStaticEntity(trav, BOTSTATICENTITY_MG42)))
		{
			...
			if (trav->health <= 0)
			{
				if (trav->melee)
				{
					...
					if ((ent->aiTeam == bs->sess.sessionTeam) && (ent->botIgnoreTime < level.time) &&
	...
	if (level.captureFlagMode)
	{
		...
		if (bs->sess.sessionTeam == level.attackingTeam)
		{
					...
					if (bestDist > 0)
					{
						...
						if (BotGoalWithinMovementAutonomy(bs, &target, BGU_LOW))
						{
							...
							if (ent && VectorDistanceSquared(bs->origin, ent->r.currentOrigin) < (384 * 384))
							{
								...
								if ((t > 1000) &&
								    (numList =
								         trap_AAS_AlternativeRouteGoals(bs->origin, bestAreaCenter, bs->tfl, altroutegoals, 40, 0)))
								{
									//pick one at random
									i = rand() % numList;
			...
			if (BotFlagAtBase(level.attackingTeam == TEAM_AXIS ? TEAM_ALLIES : TEAM_AXIS, &ent) == qtrue)
			{
				...
				if (t && ((t < 500) || !gotTarget || defendTarget || ((t / 2) < targetTime)))
				{
					...
					if (BotGoalWithinMovementAutonomy(bs, &target, BGU_MEDIUM))
					{
						...
						if (t > 600 &&
						    (numList =
						         trap_AAS_AlternativeRouteGoals(bs->origin, bs->target_goal.origin, bs->tfl, altroutegoals, 40, 0)))
						{
							//pick one at random
							i = rand() % numList;
							//make this the altrouetgoal
			...
			else
			{
				...
				//
				if ((target.entitynum > -1 && target.entitynum != bs->client) && (VectorDistanceSquared(g_entities[target.entitynum].r.currentOrigin, bs->origin) < (1600 * 1600))	/*&&
																																													   (trap_InPVS(g_entities[target.entitynum].r.currentOrigin, bs->origin)) */)
				{
					...
					// !!!
					if ((numList =
					         BotNumTeamMatesWithTarget(bs, target.entitynum, list, MAX_CLIENTS)) >= BOT_FLAG_CARRIER_DEFENDERS)
					{
		...
		else
		{
			...
			if (!gotTarget || !level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] ||
			    (level.explosiveTargets[bs->sess.sessionTeam == TEAM_AXIS ? 0 : 1] -
			     BotGetTargetExplosives(bs->sess.sessionTeam, NULL, 0, qtrue) > 0))
			{
