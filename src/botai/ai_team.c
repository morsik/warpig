...
float          *BotSortPlayersByTraveltime(int areanum, int *list, int numList)
{
	...
	for (i = 0; i < numList; i++)
	{
		...
		{
			distances[i] =
			    (float)trap_AAS_AreaTravelTimeToGoalArea(BotGetArea(list[i]), BotGetOrigin(list[i]), areanum, lbs->tfl);
		}
		else
		{
			distances[i] = 0;
...
int BotFindSparseDefendArea(bot_state_t *bs, bot_goal_t *goal, qboolean force)
{
	...
	for (i = 0; i < numTeam; i++)
	{
		if (!g_entities[teammates[i]].inuse || !(g_entities[teammates[i]].r.svFlags & SVF_BOT) ||
	...
	if (!BotSinglePlayer() && !BotCoop() && goal->entitynum >= 0 && goal->entitynum < MAX_CLIENTS &&
	    BotCarryingFlag(goal->entitynum))
	{
		...
		else
		{
			...
			if (obs->alt_goal.number && obs->alt_goal.entitynum == obs->target_goal.entitynum &&
	...
	//
	numareas =
	    trap_AAS_ListAreasInRange(targetPos, goal->areanum, maxrange, bs->tfl & ~(TFL_BARRIERJUMP | TFL_LADDER | TFL_JUMP),
	                              (float **)waypoints, 64);
...
//  char    userinfo[MAX_INFO_STRING], *str;
	...
	if (level.captureFlagMode)
	{
		...
		if ((bs->mpClass == PC_COVERTOPS) && (rand() % 3) && (level.time < level.startTime + 120000) &&
		...
		if ((bs->mpClass == PC_SOLDIER) && (rand() % 3) && (level.time < level.startTime + 30000) &&
		    (bs->mpTeam == level.attackingTeam))
		{
			//return 4; // panzer
...
qboolean BotClass_CovertOpsCheckDisguises(bot_state_t *bs, int maxTravel, bot_goal_t *goal)
{
	...
	while ((trav = G_Find(trav, FOFS(classname), "corpse")))
	{
		...
		{
			numList =
			    BotReduceListByTravelTime(list, numList, BotGetOrigin(trav->s.number), BotGetArea(trav->s.number),
...
qboolean BotClass_MedicCheckRevives(bot_state_t *bs, int maxtravel, bot_goal_t *goal, qboolean lookForBots)
{
	...
	for (i = 0; i < numTeammates; i++)
	{
		...
		{
			if (BotReduceListByTravelTime
			    (list, numList, BotGetOrigin(teammates[i]), BotGetArea(teammates[i]), BotTravelTimeToEntity(bs, teammates[i])))
			{
				continue;
			}
		}
...
*/
void BotVoiceChatAfterTeamIdleTime(int client, const char *id, int mode, int delay, qboolean voiceonly, int idleTime,
                                   qboolean forceIfDead)
{
	bot_state_t    *bs = &botstates[client];
