		...
		if (!BotIsDead(bs))
		{
			...
			if (bs->flags & BFL_MISCFLAG)
			{
				...
				if (lastCall > level.time || lastCall < level.time - 2000)
				{
					...
					{
						case PC_SOLDIER:
							BotVoiceChatAfterIdleTime(bs->client, "IamSoldier", SAY_TEAM, 1000 + rand() % 5000, BOT_SHOWTEXT,
							                          20000, qfalse);
							break;

						case PC_MEDIC:
							BotVoiceChatAfterIdleTime(bs->client, "IamMedic", SAY_TEAM, 1000 + rand() % 5000, BOT_SHOWTEXT, 20000,
							                          qfalse);
							break;

						case PC_FIELDOPS:
							BotVoiceChatAfterIdleTime(bs->client, "IamLieutenant", SAY_TEAM, 1000 + rand() % 5000, BOT_SHOWTEXT,
							                          20000, qfalse);
							break;

						case PC_ENGINEER:
							BotVoiceChatAfterIdleTime(bs->client, "IamEngineer", SAY_TEAM, 1000 + rand() % 5000, BOT_SHOWTEXT,
							                          20000, qfalse);
							break;
					}
				}
...
void AIEnter_MP_AvoidDanger(bot_state_t *bs)
{
	...
	if (!
	    (bestarea =
	         trap_AAS_AvoidDangerArea(bs->origin, bs->areanum, bs->avoid_goal.origin, BotPointAreaNum(-1, bs->avoid_goal.origin),
...
int AINode_MP_AvoidDanger(bot_state_t *bs)
{
	...
	else
	{
		    ...
		    (VectorDistanceSquared(target, goal.origin) < (SQR(trav->splashRadius + 100))))
		{
			...
			if (!
			    (bestarea =
			         trap_AAS_AvoidDangerArea(bs->origin, bs->areanum, target, BotPointAreaNum(-1, target), trav->splashRadius + 100,
			                                  bs->tfl)))
			{
...
int AINode_MP_GiveAmmo(bot_state_t *bs)
{
	...
	if (!ClientNeedsAmmo(trav->s.number))
	{
		if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(72))
		{
			...
			{
				BotVoiceChatAfterIdleTime(trav->s.number, "Thanks", SAY_TEAM, 1000 + rand() % 2000, qfalse, 3000 + rand() % 2000,
				                          qfalse);
			}
		}
...
int AINode_MP_MedicGiveHealth(bot_state_t *bs)
{
	...
	if ((bs->last_fire != level.time) &&
	    (bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] < (int)(0.8 * GetAmmoTableData(bs->cur_ps.weapon)->maxclip))
	    && bs->cur_ps.ammo[BG_FindAmmoForWeapon(bs->cur_ps.weapon)])
	{
		trap_EA_Reload(bs->client);
	}
...
int AINode_MP_MedicRevive(bot_state_t *bs)
{
	...
	if (
	    ...
	    !trav->inuse || !trav->client || (trav->client->ps.pm_flags & PMF_LIMBO))
	{
		// just look for a goal
		bs->ignore_specialgoal_time = 0;
		BotDefaultNode(bs);
	...
	if (trav->client->ps.pm_type != PM_DEAD)
	{
		if (VectorDistanceSquared(bs->origin, goal.origin) < SQR(72))
		{
			...
			if (trav->r.svFlags & SVF_BOT)
			{
				BotVoiceChatAfterIdleTime(trav->s.number, "Thanks", SAY_TEAM, 1000 + rand() % 2000, qfalse, 4000 + rand() % 2000,
				                          qfalse);
			}
		}
	...
	if (goal.flags & GFL_LEADER)
	{
		...
		if (bs->leader > -1)
		{
			if (!g_entities[bs->leader].inuse ||
			    !g_entities[bs->leader].client || (g_entities[bs->leader].client->ps.pm_flags & PMF_LIMBO))
			{
				bs->leader = -1;
				BotDefaultNode(bs);
				return qfalse;
			...
			else if ((VectorLengthSquared(g_entities[bs->leader].client->ps.velocity) < SQR(10)) &&
			         (VectorLengthSquared(bs->cur_ps.velocity) < SQR(10)))
			{
				if (!(g_entities[bs->leader].r.svFlags & SVF_BOT))
				{
					BotVoiceChatAfterIdleTime(bs->client, "WhereTo", SAY_BUDDY, 1000 + rand() % 3000, BOT_SHOWTEXT, 12000,
					                          qfalse);
				}
			}
	...
	else
	{
		...
		if ((ent->s.eType == ET_OID_TRIGGER) && (objtarg = ent->target_ent))
		{
			if (objtarg->spawnflags & 64)
			{
				...
				if (bs->sess.playerType == PC_FIELDOPS &&
				    (level.time - bs->cur_ps.classWeaponTime > (level.lieutenantChargeTime[bs->sess.sessionTeam - 1] * 0.5f)))
				{
		...
		else
		{
			...
			if (goal.entitynum > -1 && goal.entitynum < level.maxclients && BotCarryingFlag(goal.entitynum))
			{
				...
				if (!setdir)
				{
					...
					VectorAdd(g_entities[bs->target_goal.entitynum].r.absmax, g_entities[bs->target_goal.entitynum].r.absmin,
					          target);
...
int AINode_MP_TouchTarget(bot_state_t *bs)
{
	...
	*/
			    ...
			    (numList = BotNumTeamMatesWithTarget(bs, bs->client, list, MAX_CLIENTS)))
			{
				...
				for (i = 0; i < numList; i++)
				{
					...
					if ((VectorDistanceSquared(tbs->origin, tbs->defendgoal.origin) < SQR(1500)) &&
					    ...
					    (trap_AAS_AreaTravelTimeToGoalArea(tbs->defendgoal.areanum, tbs->defendgoal.origin, goal.areanum, bs->tfl)
					     < trap_AAS_AreaTravelTimeToGoalArea(bs->areanum, bs->origin, bs->target_goal.areanum, bs->tfl)) &&
	...
	else
	{
		...
		if (bs->enemy < 0 && !(bs->flags & BFL_IDEALVIEWSET))
		{
			if (ent->s.eType == ET_TRIGGER_MULTIPLE && trap_InPVS(bs->origin, goal.origin) &&
	...
	if (goalDist < SQR(32))
	{
		...
		if (!trav)
		{
			...
			if (BotWeaponCharged(bs, WP_SATCHEL))
			{
				...
				// hold fire
				if (!VectorLengthSquared(bs->velocity) && fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 &&
		...
		if (!trav)
		{
			...
			if (BotWeaponCharged(bs, WP_DYNAMITE))
			{
				...
				if (rand() % 2 && !VectorLengthSquared(bs->velocity) &&
				    fabs(bs->viewangles[PITCH] - bs->ideal_viewangles[PITCH]) < 2 && bs->cur_ps.weapon == WP_DYNAMITE &&
...
int AINode_MP_Battle_Fight(bot_state_t *bs)
{
	...
	if ((bs->weaponnum == WP_KNIFE) || (enemydist > SQR(BotWeaponRange(bs, bs->weaponnum))) ||
	...
	if (BotEntityVisible(bs->entitynum, bs->eye, bs->viewangles, 360, bs->enemy, NULL))
	{
		...
		if (bs->cur_ps.groundEntityNum != ENTITYNUM_NONE || bs->cur_ps.velocity[2] < 0)
		{
			...
			if ((bs->weaponnum != WP_KNIFE) &&
			    ((!BotCarryingFlag(bs->enemy) && (enemydist < SQR(BotWeaponRange(bs, bs->weaponnum)))) || (enemydist < SQR(64))))
			{
	...
	if (bs->flags & BFL_MISCFLAG)
	{
		...
		if (bs->last_attackShout < level.time - 250)
		{
			...
			if (trap_AAS_PredictClientMovement
			    (&move, bs->client, bs->origin, -1, qfalse, dir, vec3_origin, checkFrames, 0, checkFrametime,
