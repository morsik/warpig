...
qboolean Bot_ScriptAction_SetAccumToPlayerCount(bot_state_t *bs, char *params)
{
	...
	{
		Bot_ScriptError(bs, "accum buffer index out of range, %s found (range is 0 - %i): SetAccumToPlayerCount %s", token,
		                MAX_SCRIPT_ACCUM_BUFFERS - 1, params);
	}
...
qboolean Bot_ScriptAction_Logging(bot_state_t *bs, char *params)
{
	...
	if (!Q_stricmp(params, "ON"))
	{
		...
		Q_strncpyz(filename,
		           va("BotLog_%s_[%i]_[%4i_%2i_%2i]_[%2i_%2i_%2i].txt", g_entities[bs->client].aiName, bs->client,
		              1900 + localTime->tm_year, 1 + localTime->tm_mon, 1 + localTime->tm_mday, localTime->tm_hour,
		              localTime->tm_min, localTime->tm_sec), sizeof(filename));
...
qboolean Bot_ScriptAction_SetAmmoAmount(bot_state_t *bs, char *params)
{
	...
	weapon =
	    Bot_GetWeaponForClassAndTeam(g_entities[bs->client].client->sess.playerType,
	...
	if (!COM_BitCheck(bs->cur_ps.weapons, weapon))
	{
		Bot_ScriptError(bs, "Bot_ScriptAction_SetAmmoAmount: Bot %s does not have weapon %s", g_entities[bs->client].aiName,
		                token);
	}
