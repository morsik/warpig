...
void BotChooseWeapon(bot_state_t *bs)
{
	...
	if (bs->cur_ps.weaponstate == WEAPON_RAISING || bs->cur_ps.weaponstate == WEAPON_DROPPING || bs->cur_ps.weaponDelay)
	{
	...
	for (i = 0; i < 2; i++)
	{
		...
		*///
		...
		if (trap_AAS_PredictClientMovement
		    (&move, bs->client, bs->origin, -1, qtrue, sideward, vec3_origin, 10, 0, 0.1, SE_GAP | SE_LEAVEGROUND | SE_STUCK, -1,
		     qfalse))
		{
...
void sAdjustPointTowardsPlayer(vec3_t playerLoc, vec3_t endPos, qboolean shouldLoop,
...
qboolean BotDangerousGoal(bot_state_t *bs, bot_goal_t *goal)
{
	...
	for (i = 0; i < dangerEntsCount; i++)
	{
			...
			case ET_MISSILE:
				switch (trav->methodOfDeath)
				{
					...
					case MOD_GRENADE_LAUNCHER:
						...
						if (!G_PredictMissile
						    (trav, trav->nextthink - level.time, bangPos, trav->methodOfDeath == MOD_PANZERFAUST ? qfalse : qtrue))
						{
							// not dangerous
							continue;
						}
		...
		if (trav->s.eType == ET_CONSTRUCTIBLE || (VectorDistanceSquared(bangPos, goal->origin) < SQR(radius + 90)) ||
...
qboolean ChangeBotAlertState(bot_state_t *bs, aistateEnum_t newAlertState, qboolean force)
{
	if (force)
	{
		// should only be used by script action
	...
	if (newAlertState != bs->alertState && level.time > bs->alertStateAllowChangeTime
			...
			if (!(dist > 100 && VectorLengthSquared(dir) < SQR(32)))
			{
				    ...
				    bs->cur_ps.weaponstate == WEAPON_READY)
				{
					...
					trap_AAS_PredictClientMovement(&move, bs->enemy, origin,
					                               ...
					                               dir, cmdmove, 0, dist * 10 / wi.speed, 0.1, 0, 0, qfalse);
...
qboolean BotCheckAttack(bot_state_t *bs)
{
	...
	if ((bs->last_botcheckattack_weapon == bs->weaponnum) && (bs->enemy == bs->last_botcheckattack_enemy) &&
...
passed:
	...
	if (bs->cur_ps.weapon == WP_GRENADE_LAUNCHER || bs->cur_ps.weapon == WP_GRENADE_PINEAPPLE)
	{
		if (bs->cur_ps.grenadeTimeLeft &&
		    ((bs->inventory[INVENTORY_HEALTH] < 15) || (bs->cur_ps.grenadeTimeLeft < (700 + (int)(4000.0 * (1.0 - aimskill))))))
		{
			// release grenade
		}
		else
...
void BotRecordPain(int client, int enemy, int mod)
{
	...
	if (bs->defendgoal.entitynum >= level.maxclients)
	{
		...
		if (Q_stricmp(targ->classname, "team_CTF_redflag") &&
		    ...
		    Q_stricmp(targ->classname, "trigger_flagonly") && Q_stricmp(targ->classname, "team_WOLF_checkpoint"))
		{
			return;
		}
		...
		if (!BotSinglePlayer() && !BotCoop())
		{
			BotVoiceChatAfterIdleTime(bs->client, "TakingFire", SAY_TEAM, 1000 + rand() % 1000, qfalse, 5000 + rand() % 4000,
			                          qfalse);
		}
	}
...
void BotCheckVoiceChats(bot_state_t *bs)
{
	if (VectorLengthSquared(bs->cur_ps.velocity) < SQR(10))
	{
		...
		{
			BotVoiceChatAfterIdleTime(bs->client, "NeedAmmo", SAY_TEAM, 2000 + rand() % 10000, qfalse, 40000 + rand() % 15000,
			                          qfalse);
		}

		// do we need health?
		// TAT 10/8/2002 - Medics only ask for health if they are dead
		if ((bs->sess.playerType == PC_MEDIC && BotHealthScale(bs->client) <= 0.0) ||
...
qboolean BotSeekCover(bot_state_t *bs)
{
	...
	//
	area =
	    trap_AAS_NearestHideArea(bs->client, bs->origin, bs->areanum, bs->enemy, g_entities[bs->enemy].r.currentOrigin, enemyarea,
	                             bs->tfl, BotGetMovementAutonomyRange(bs, NULL), autonomyPos);
...
#endif
	{
		...
		switch (move.stopevent)
		{
			...
			case SE_HITGROUNDAREA:
				...
				if (tr.fraction < .99f && VectorDistanceSquared(bs->origin, tr.endpos) < SQR(30) &&
