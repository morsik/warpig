	...
	if (cg_gameType.integer == GT_WOLF_CAMPAIGN)
	{
		...
		for (i = 0; i < cgs.campaignData.mapCount; i++)
		{
			...
			CG_Text_Paint_Ext(button->rect.x + 8, y + 11, 0.19f, 0.19f, clrTxtBck,
			                  va("%i. %s", i + 1, cgs.campaignData.arenas[i].longname), 0, 0, 0, &cgs.media.limboFont2);
	...
	if (cg_gameType.integer == GT_WOLF_CAMPAIGN)
	{
		...
		if (cgs.campaignData.mapTC[0][0] && cgs.campaignData.mapTC[1][0])
		{
			...
			for (i = cgs.campaignData.mapCount - 1; i >= 0; i--)
			{
				...
				if (x + 10 + w > button->rect.x + button->rect.w)
				{
					CG_Text_Paint_Ext(x - w - 10, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0,
					                  &cgs.media.limboFont2);
				}
				else
				{
					CG_Text_Paint_Ext(x + 10, y + 3, 0.2f, 0.2f, colorWhite, cgs.campaignData.arenas[i].longname, 0, 0, 0,
					                  &cgs.media.limboFont2);
				}
			}
	...
	for (j = 0; j < 2; j++)
	{
		for (i = 0; i <= SK_NUM_SKILLS; i++)
		{
			...
			if (*str)
			{
				...
				CG_Text_Paint_Ext(button->rect.x + 100 + skillPositions[i] - (w * 0.5f), button->rect.y + (j * 11), 0.2f, 0.2f,
...
void CG_Debriefing2TeamSkillXP_Draw(panel_button_t *button)
{
	...
	for (i = 0; i <= SK_NUM_SKILLS; i++)
	{
		...
		CG_Text_Paint_Ext(button->rect.x + 100 + skillPositions[i] - (w * 0.5f), button->rect.y + 11, scale, scale, clrTxtBck,
