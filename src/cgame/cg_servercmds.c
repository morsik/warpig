...
// jpw
	...
	// we really should clear more parts of 2cg here and stop sounds
	cg.v_dmg_time = 0;
	cg.v_noFireTime = 0;
	cg.v_fireTime = 0;
...
//  BG_cleanName(cgs.clientinfo[gs->nClientID].name, strName, sizeof(strName), qfalse);
	...
	if (skillMask != 0)
	{
		...
		for (i = SK_BATTLE_SENSE; i < SK_NUM_SKILLS; i++)
		{
			...
			{
				Q_strncpyz(gs->strSkillz[gs->cSkills++], va("%-15s %3d %s %12d", skillNames[i], ci->skill[i], str, ci->medals[i]),
				           sizeof(gs->strSkillz[0]));
			}
			else
			{
				Q_strncpyz(gs->strSkillz[gs->cSkills++], va("%-15s %3d %s", skillNames[i], ci->skill[i], str),
				           sizeof(gs->strSkillz[0]));
			}
		}
	}
