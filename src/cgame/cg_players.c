...
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3])
{
	...
	// morsik: add lean to head and body
//	if (cent /* SOME PLAYER THAT LEANS */)
/*	{
		torsoAngles[2] += cg.snap->ps.leanf;
		headAngles[2]  += cg.snap->ps.leanf;
	}*/
...
//  float           gumsflappin = 0;    // talking amplitude
...

	{
		...
		if (distSquared > Square(384.0f))
		{
			renderfx |= RF_MINLIGHT;

			distSquared -= Square(384.0f);

			if (distSquared > Square(768.0f))
			{
				hilightIntensity = 1.0f;
			}
			else
			{
				hilightIntensity = 1.0f * (distSquared / Square(768.0f));
			}
...
// DEBUG
	...
	/*{
	   ...
	   maxs[2] = maxs[2] - (cg.predictedPlayerState.ViewHeight - PRONE_VIEWHEIGHT + 8);
