...
void G_LogDeath(gentity_t *ent, weapon_t weap)
{
	...
	trap_PbStat(ent - g_entities, "death", va("%d %d %d", ent->client->sess.sessionTeam, ent->client->sess.playerType, weap));
}
...
void G_LogKill(gentity_t *ent, weapon_t weap)
{
	...
	trap_PbStat(ent - g_entities, "kill", va("%d %d %d", ent->client->sess.sessionTeam, ent->client->sess.playerType, weap));
}
